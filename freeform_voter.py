...
class FreeformVoter:
    ...
    def combine(self, input):
      colors = [[0xC1, 0xFF, 0xC1], [0xBC, 0xEE, 0x68], [0x00, 0xCD, 0xCD], [0x76, 0xEE, 0xC6], [0xEE, 0xDF, 0xCC], [0xEE, 0xC5, 0x91], [0xB2, 0x3A, 0xEE], [0x00, 0xFF, 0xFF], [0xC1, 0xCD, 0xCD]]
      result = []
      for level in input:
        temp = []
        for i in colors:
        ...
        for color in level:
          for action in temp:
            if color in action:
              action.append(color)
              found = True
        result.append(copy.deepcopy(sum(map(lambda x: x[1:], temp), [])))
      return result
    ...
    def _test_trolley(self, model, env_creator, granularity, on_track, on_track_list, sequence_number, filename):
        ...
        #print (outcome_pic)
        labels = ['Down', 'Up', 'Right', 'Left', 'Nothing', 'Lie', 'Torture', 'Emphasis', 'Trial']
